...
const resolveThemeValues = (style, theme = {}) => {
  ...
  Object.entries(style).forEach(([property, value]) => {
    const themeValueResolver = themeMapping[property];
    if (typeof value === 'string' && themeValueResolver) {
      const isNegative = NEGATIVE_REGEXP.test(value);
      if (isNegative) {
        value = value.split(NEGATIVE_REGEXP)[1].trim();
      }
      const themeValue = getThemeValue(themeValueResolver(theme), value);
...
const getThemeValue = (object, key) => {
  const value = key.split('.').reduce((value, index) => {
    return isPlainObject(value) ? value[index] : undefined;
  }, object);
  return value || key;
};
